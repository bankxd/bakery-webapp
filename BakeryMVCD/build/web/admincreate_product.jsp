            ...
            <editor-fold defaultstate="collapsed" desc="Sign In">
                <div id="editacc" >
                    <form action="adminproduct" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="w3-container">
                            <label for="uname"><b>Product Name</b></label></br>
                            <input type="text" placeholder="Enter Name" name="prodname" required></br></br>

                            <label for="psw"><b>Description</b></label></br>
                            <input type="text" placeholder="Enter Description" name="proddesc" required></br></br>

                            <label for="psw"><b>Product Warnings</b></label></br>
                            <input type="text" placeholder="Enter Warnings" name="prodwar" required></br></br>

                            <label for="psw"><b>Price</b></label></br>
                            <input type="text" placeholder="Enter Price" name="prodprice" required></br></br>

                            <label for="psw"><b>Discount</b></label></br>
                            <input type="text" placeholder="Enter Discount" name="proddisc"required></br></br>

                            <label for="psw"><b>Ingredients</b></label></br>
                            <% ArrayList<Ingredient> ingArr = (ArrayList<Ingredient>) new IngredientsClient().getAllIngredients(); %>
                            <table>
                                <% for (Ingredient ing : ingArr) {%>
                                <tr>
                                    <td><input type="checkbox" placeholder="Ingredients" name="ingaddID" value="<%= ing.getIngredientId()%>"><%= ing.getIngredientName()%></td>
                                    <td><input type="number" placeholder="0" name="quanaddname" ></td>
                                </tr>
                                <% }%>
                            </table>
                            ...
                            <table>
                                <% for (Category selcat : catArr) {%>
                                <tr>
                                    <td><input type="radio" name="catradio" value="<%= selcat.getCategoryID()%>" required><%= selcat.getCategoryName()%></td>
                                </tr>
                                <% }%>
                            </table>
                            <label for="uname"><b>Photo Name</b></label></br>
                            <input type="text" placeholder="Enter Photo" name="prodphotoName" required ></br></br>
                            <label for="uname"><b>Photo</b></label></br>
                            <input type="file" placeholder="Enter Photo" name="prodphoto" ></br></br>
                            <% session.setAttribute("ingArr", ingArr);%>
                            <button type="submit" name="prodbut" value="Add Product">Add new Product</button>
                        </div>
                    </form>
                </div>
            </editor-fold>
